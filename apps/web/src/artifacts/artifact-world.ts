...
import type { Alliance, MatchPhase, MatchState, MatchRobotSnapshot } from '@ftc-sim/game-decode';
...
import { barrierToBodyDef, ROBOT_BODY_ID } from '../robot/physics-scene';
...
export class ArtifactWorld {
  ...
  /** Update goal barriers without rebuilding Rapier world or resetting artifacts. */
  syncBarriers(barriers: EditableBarrier[]): void {
  ...
  reset(staging: StagedArtifactLayout[], startPose: Pose, npcRobots?: NpcRobotSync[]): void {
    if (!this.initialized) return;
    this.sim.init(staging);
    this.randomizeMotif();
