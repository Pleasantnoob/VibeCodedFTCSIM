...
function zipUrl(version: string): string {
  const arch = macArch();
  return `https://github.com/${REPO}/releases/download/v${version}/FTC-Sim-Player-${version}-mac-${arch}.zip`;
}

function zipDest(version: string): string {
  const arch = macArch();
  return join(downloadsDir(), `FTC-Sim-Player-${version}-mac-${arch}.zip`);
}
...
/** Check GitHub Releases (latest-mac.yml) and download zip with progress when user accepts. */
export function setupAutoUpdater(getLauncherWindow: () => BrowserWindow | null): void {
  ...
  autoUpdater.on('update-available', (info) => {
    ...
    const dialogOptions = {
      ...
      message: `FTC Sim Player ${info.version} is available.`,
      detail: `You have ${app.getVersion()}. Download the zip and replace FTC Sim Player in Applications.`,
  ...
  try {
    ...
    const { response } = launcher
      ? await dialog.showMessageBox(launcher, {
          ...
          detail: `Saved to ${path}\n\nUnzip and drag FTC Sim Player to Applications to update.`,
